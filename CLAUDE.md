...
---
## 9. Авторское (только ваш IWE)

> Этот раздел — пустая болванка L3 (см. таблицу `CONTRIBUTING.md` Three Layers).
> `update.sh` НЕ трогает §9 — пишите сюда свои блокирующие правила, различения,
> соглашения об именовании и read-only репозитории. Они переживут любое обновление шаблона.

### Блокирующие (авторские)

### Различения (авторские)

> Личные различения — в `.claude/rules/distinctions.md`, зона AUTHOR-ONLY (не затирается `update.sh`).
